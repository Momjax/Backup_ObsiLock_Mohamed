...
```mermaid
flowchart LR
    subgraph Client ["Client Lourd Desktop"]
        JavaFX("☕ JavaFX 17<br/>(Interface ObsiLock)")
    end

    subgraph Server ["VPS iris.a3n.fr (Docker)"]
        Traefik("🌐 Traefik<br/>(Reverse Proxy HTTPS)")
        API("🐘 API PHP 8 (Slim)<br/>Authentification + Chiffrement")
        DB[("🐬 MariaDB 10<br/>(Métadonnées)")]
        Storage[("📂 /storage/uploads<br/>(Fichiers chiffrés)")]
    end

    JavaFX -- "Requêtes HTTPS<br/>(JSON + JWT)" --> Traefik
    Traefik -- "Route vers conteneur API" --> API
    API <--> DB
    API -- "Flux de streaming<br/>LibSodium" <--> Storage
```
...
```mermaid
erDiagram
    USERS ||--|{ FOLDERS : "possède (user_id)"
    USERS ||--|{ FILES : "possède (user_id)"
    USERS ||--|{ SHARES : "partage (user_id)"
    
    FOLDERS ||--o{ FOLDERS : "contient (parent_id)"
    FOLDERS ||--o{ FILES : "contient (folder_id)"
    
    FILES ||--|{ FILE_VERSIONS : "est versionné (file_id)"
    FILES ||--o{ SHARES : "partagé via (file_id)"
    
    SHARES ||--o{ DOWNLOADS_LOG : "suivi par (share_id)"
...
```mermaid
flowchart LR
    A[Utilisateur] -->|1| B(S'authentifier JWT)
    A -->|2| C(Uploader Fichiers Chiffrés)
    A -->|3| D(Gérer l'arborescence)
    A -->|4| E(Créer un lien de Partage)
    
    AD[Administrateur]
    AD -->|hérite| A
    AD --> F(Administrer Quotas Globaux)
```
...
classDiagram
    class App {
        ...
        +toggleTheme(Scene scene)
    }
    class ApiClient {
        -String baseUrl
        -String authToken
        +login(email, pwd) String
...
gantt
    ...
    Sécurité Rate Limit     :done,    des6, 2026-02-14, 1d
    section Front & Déploiement
    Tests Unitaires         :done,    des7, 2026-02-15, 1d
    Docker & Prod           :done,    des8, 2026-03-22, 1d
```
